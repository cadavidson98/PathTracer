...
struct Image {
  ...
  Image(const char* fname) {
    ...
    for (int i = 0; i < width; i++){
      for (int j = 0; j < height; j++){
        ...
        pixels[(i+j*width)] = Color(data[4*(i+j*width)+0]/255.0f,data[4*(i+j*width)+1]/255.0f,data[4*(i+j*width)+2]/255.0f);
      }
    }
  ...
  Color sample(float i, float j){
      int u = static_cast<int>(i * width);
      int v = static_cast<int>(j * height);
      return getPixel(u, v);
  };
  ...
  void write(const char* fname){
    ...
    int lastc = static_cast<int>(strlen(fname));
